...
def read_midi_files():
    ...
    for i in range(len(note_list)):
        ...
        while(len(note_list[i][0])<4 and len(note_list[i][1])<4):
            del note_list[i][0]
        while(len(note_list[i][-1])>4 and len(note_list[i][-2])>4):
            del note_list[i][-1]

        for k in range(len(note_list[i])):
            total_note.append(note_list[i][k])
    ...
    for i in range(len(notes)):
            ...
            if len(notes[i])>3:
                pre_chord=notes[i]
            if (0<len(pre_note)<4 and 0<len(notes[i])<4):
                notess.append(pre_chord)
            pre_note=notes[i]
