...
def samples(epoch, header, num_chars, vocab_size, idx_to_char, char_to_idx):
    prediction = []
...
def predictModel(numChars,header):
    vocab_size, idx_to_char, char_to_idx = char_idx_char_mappings()
    modelWeights_epoch = [100]
    for epochs in modelWeights_epoch:
        prediction = samples(epochs, header, numChars, vocab_size, idx_to_char, char_to_idx)
    return prediction
